...
module "eks" {
  ...
  security_group_ids = ["${aws_security_group.uat-cluster_to_nodes.id}"]
  ...
  worker_groups = [
    {
      ...
      additional_security_group_ids = [aws_security_group.worker_group_mgmt_one.id, aws_security_group.uat-node.id ]
    },
    # {
    #   name                          = "worker-group-2"
